...
img.setAttribute('src','img/rainbow-icon.png');
...
function createColorBox(colorString, numericSize) {
  ...
  a.classList.add('height')
  a.classList.add('background-color')
  a.style.backgroundColor = colorString
  a.style.width = numericSize + "px"
  a.style.height = numericSize +"px"
  return a
}
console.log(createColorBox('red', 100))
...
function getElementWidth(domElement){
  let a = domElement.getBoundingClientRect();
  return a.width;
}
...
*/
function renderPaletteRow(colorStringArray, parentDomElement){
  let a = document.createElement("div")
  for (let i=0; i < colorStringArray.length; i++) {
    a.appendChild(createColorBox(colorStringArray[i], getElementWidth(parentDomElement)/colorStringArray.length))
  }
  parentDomElement.appendChild(a)
}
...
function renderPaletteTable(){
  ...
  Object.keys(COLORS_9).forEach(function(paletteName){
    renderPaletteRow(COLORS_9[String(paletteName)],main)
  })
}
renderPaletteTable()
